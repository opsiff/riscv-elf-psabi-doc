...
The function signature based landing pad PLT style occupies two 32 byte entries:
...
The label value for the function signature based labeling scheme landing pad is computed from the
...
<<itanium-cxx-abi>>, and the function signature will use the "Compression" rule
defined in  _Itanium {Cpp} ABI_, the value is taken from the lower 20 bits of
the MD5 hash result of the function signature string.
...
- {Cpp} member functions should use the "Pointer-to-member types" mangling rule
  ...
  - The return type of a class member function should mangle to `void *` if it
    is a pointer or reference to a non-primitive type. A pointer to a pointer
    of a non-primitive type is not included in this rule.
  - Class destructors should use the signature `void (*)(void*)` (`FvPvE`).
  - `<exception-sepc>` should be ignored.
  - Static function is following the rule as normal function.
...
- Function with an empty parameter list are treated as `void` (`v`).

NOTE: The special rule for the return type of class member functions is defined
      to handle covariant return types.
...
- `OtherBase::memfunc2` is encoded as `M9OtherBaseFviE`.
- `Derived::memfunc3` and `DerivedDerived::memfunc3` are both encoded as
  `M7DerivedFvdE`.
- `Derived::memfunc4` is encoded as `M7DerivedFvvE`.
- `DerivedDerived::memfunc2` is encoded as `M4BaseFviE`, and the thunk function
   for `OtherBase::memfunc2` will be `M9OtherBaseFviE`.
